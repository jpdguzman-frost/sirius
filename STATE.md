...
- 2026-09-12 — **`w4-server` merged to main** (`55cf976`, JP's word) now block 9 has shipped; still **INERT** — no route, no caller, `unit_label` outside `registryFields()`, so nothing writes or reconciles it. VALIDATE green: typecheck · lint · 1,779 tests UTC + Manila (one rule-5 loopback 404, clean on the re-run) · 274 date-sensitive under New_York. Branch kept, unpushed. Log: `docs/history/state-log/2026-09-12.md`.
