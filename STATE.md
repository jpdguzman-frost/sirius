...
| 18c | **Sprint Schedules SCREEN rebuilt** (owls #72/#73, frame 731:98513) — first build under the workflow law: 4-agent fleet, adversarial review (15 confirmed findings incl. the sprint-id regeneration data-loss bug), simplify, browser E2E green | **DEPLOYED 2026-08-28** (`9128017`, JP's ship) — #72/#73 acked, miles informed (#60); Deadlines part 2 (#74) is next; **spot-fix built 2026-08-28** (JP's checks vs 733:108505 + 731:100230): deadline rule pins left for past dates, placement is hover-gated + one-click draft place — 12 review findings fixed, 1090 green dual-TZ, 11 revert proofs, E2E real-pointer — **DEPLOYED 2026-08-28** (`e7ad007`, JP's go);**Test-board group join repaired 2026-08-28** (`c7f9d3c`): create-test-board's regex bug orphaned its work cards (MC-909→MC-5); repaired to the live board's measured lane/title/label principles via ARES, ARES roster fixed by JP, full add→inherit→place proven on rt-test (deadline+urgency inherit from the MC group). |
