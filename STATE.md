...
- 2026-09-12 — **The swept document set ADOPTED** (JP: yes, and *follow theirs* on numbering): **BRD v3.0** + the swept engineering doc + a new pilot security doc are the held copies. Closes the BRD §9 blocker open since 2026-08-04. **Their numbering adopted twice**: ACs (1–26 theirs, ours → AC-27–AC-30) and requirements (**FR-11 → FR-3.7–FR-3.10, FR-12 → FR-6.9–FR-6.14**; five sub-rules stay local, FR-11.3/11.4 HELD). A banner carries what the sweep does NOT override: the engineering doc's Postgres §1 (Stack wins). **`spec.md` RE-CONVERTED the same day** (JP: start now) — v3.0's FRs and BRs verbatim, user stories and entities on the work-card unit, **three GAPs marked in place** (FR-3.3's status values · FR-7.3's forecast axis · BR-10's source of state), each naming what the build follows meanwhile. #98–#113 acked. Log: `docs/history/state-log/2026-09-12.md`.
