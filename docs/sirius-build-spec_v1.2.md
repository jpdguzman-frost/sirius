...
Six tabs, icon plus label: Requests, Pipeline, Sprint Schedules, Deadlines,
Forecast, and **Admin** — user access management, visible to admins only.
...
**Editable here**, unlike on Requests, and the edit **writes the Trello due
date** — set or clear. Default time 17:00 Manila; an existing time of day is
preserved.

**There is no Sirius-local override layer.** Precedence is **Trello → sheet** by
construction: writing to Trello means a change made in either place flows back
without a reconciliation rule. A date changed by a designer in Trello appears
here on the next push, and vice versa.

Applies whether or not the card has a matching intake row — plenty of cards are
internal work with no request behind them and still need a date.

Tooltip names the source. `clear` removes the Trello due date rather than
falling back to a local value.

### 4.3 Urgency — Interaction

**One of exactly two writes in the system**, the other being the card due date
(§4.2). Nothing else is written anywhere. **Adding a third requires a governance
amendment**, not a code change — the enumerated-write posture is quoted in the
BRD, the security readiness doc and the vendor assessment.

Adds or removes a Trello label named `Urgent`;
...
delivery — landing in whichever week each falls in.
...
sketch week and 4 again in its render week. Not double-counting: sketching the
work and rendering it later are separate work, and one unit means the same thing
on Sprint Schedules and Deadlines.
