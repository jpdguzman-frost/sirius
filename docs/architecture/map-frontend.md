...
- `frontend/scripts/50-gantt-geometry.js` — workday x-axis: TOTAL_UNITS, dayIndex, clampUnits, pctOf/unitPct, itemBar/itemPhase (the single urgency-coloured bar; phaseRun is retired), barLeftAt (the resting bar's left, MIN_GRAB slide included), dlDayPlaceable (block 9: the client half of the Deadlines drag's four bounds — week · sprint · deadline; holidays are the server's) + app.set of itemBar/deadlineTick/sprintLength/dlDayPlaceable. dayAtX/plusLeft/placeable removed 2026-09-11 (#88: day controls left Sprint Schedules).
...
- `frontend/scripts/90-events.js` — interaction layer: selectTab, resetForProjectSwitch, applyRequestFilter, ONE indivisible `app.on({...})` handler map (tabs, filters, notes, admin, menus, the due popover, sprints modal, the Deadlines month + lane expand, plotHover/plotLeave/plotPlace, barDragStart/barDragMove/barDragEnd/barDragCancel — block 7's day-grain placement and pointer bar-drag), patchUrl/bumpWeek, announceArrival (the arrival affordance).
...
- `frontend/templates/partials/20-deadline-card.html` — the Deadlines card partial (owl #74, node 810:122333): urgent quote bar as the frame's path, the four badge kinds, the three-line title, the links row; registered top-level like dueCalendar. Block 9: the expanded card is the drag source (pointerdown → dlDragStart, keydown → dlKey, tabindex 0, role button, data-row) and wears dragging/refused from dlDrag; collapsed cards carry none of it.
...
- `frontend/templates/partials/40-displaced-notice.html` — displacedNotice (block 9, owl #90): the post-save banner naming each card a sprint re-date moved to Outside any sprint (display id · title · day) with its Dismiss; rendered between the legend and the scroller when sprintDisplaced is non-null.
...
- `frontend/templates/views/50-deadlines.html` — Deadlines tab: the month navigator and the week lanes over the schedule's own rows; the ONE day control (block 9, #88/#89): an expanded lane is `.dllane[data-week]` with five `.dlday[data-day]` drop columns (`target` on the allowed day) — the drag never leaves the lane.
