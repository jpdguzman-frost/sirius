...
- `frontend/scripts/50-gantt-geometry.js` — workday x-axis: TOTAL_UNITS, dayIndex, clampUnits, pctOf/unitPct, dayAtX (block 7; weekAtX deleted 2026-09-09, no callers left), itemBar/itemPhase (the single urgency-coloured bar; phaseRun is retired), barLeftAt (the drag preview's left, MIN_GRAB slide included), placeable (the affordance guard) + app.set of itemBar/deadlineTick/plusLeft/sprintLength/placeable.
