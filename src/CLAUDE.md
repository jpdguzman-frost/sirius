...
Auto-loaded when working under `src/`. Where a rule has an authoritative home
this file points in one line — it never restates.
...
1. **`project_id` everywhere** — every collection carries it, every query
   ...
   [src/services/calendar-sync.ts header].
...
2. **Zod at every API boundary** [root CLAUDE.md §Stack]. Every route that
   ...
   GET-only today. Each route file exports one `<name>Router()` factory
   returning an Express `Router` [src/routes/*.ts; src/auth/routes.ts].
...
3. **Writes to source systems**: the only Trello write paths in `src/` are
   ...
   `writeGuards()`; any new registry entry routes through it.
...
8. **Probing a running instance** (host or laptop, `npx tsx -e`): the Mongo
   ...
   the Manila "today" string is `manilaToday()` from
   `src/services/pipeline.ts`; and `loadPipeline(projectId, today,
   weeklyCapacity)` takes **THREE args with `projectId` as an
   `ObjectId`** — pass a string and it returns 0 rows silently. Remote node
   path and the local dev command: `docs/operations/deploy.md`.
...
silently re-decided [docs/architecture/context-architecture.md, principle 5].
