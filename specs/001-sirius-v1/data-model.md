...
| `frost_notes` *(added 2026-08-12, FR-3.7–FR-3.10; local FR-11 until 2026-09-12)* | project_id, mc_number, remark, clarify(false), clarify_reason, updated_by(lowercase), updated_at · one per request; never written to the sheet | (project_id, mc_number) | — |
| `milestone_day_plan` *(added 2026-08-12, FR-6.9–FR-6.14; local FR-12 until 2026-09-12)* | project_id, trello_card_id, phase(sketch\|render), day(DATE_ONLY, must sit inside the milestone's week, non-holiday), week(DATE_ONLY — the Monday the placement was made for; a mismatch with the milestone's computed week = lapsed, reads as absent), set_by(lowercase), set_at · absent row = follow the forecast | (project_id, trello_card_id, phase) | — |
