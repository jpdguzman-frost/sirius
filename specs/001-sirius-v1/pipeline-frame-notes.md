...
| ID | Ruling | Shipped |
|---|---|---|
| **R-warn-i** | **Which corner is squared when the card flips ABOVE the icon.** The annotation specifies the anchored-below case only (top-left `0`, other three 8px). Default taken: **mirror it** — bottom-left `0`, other three 8px — so the card still reads as growing out of the icon rather than floating over it. Carried by `.warnpop.flip`, driven by `warnPopPos.up`. **Flagged to Miles.** | Yes — `.warnpop` and `.warnpop.flip` each assert four explicit values with the square on the gap side |
| **R-warn-j** | **Close delay = 150ms.** Not specified anywhere. Long enough to cross the 4px gap, short enough not to feel sticky. Named constant `WARN_CLOSE_MS`, so tuning it is one line. **Flagged to Miles as a number he may want to change.** | Yes — a test refuses a literal at the call site |
| **R-warn-k** | **Dark variant not built.** The annotation says the dark variant is "in use on Sprint Schedules (#0f172a)". **There is no warning card on Sprint Schedules in this build** — the same class of Figma-side statement as R-warn-e's. Nothing invented, no unused dark recipe added. **Mismatch recorded for the product team.** | n/a — nothing built, deliberately |
...
| **R-warn-q** | **Figma-side filler, no action.** *"REMAINING FILLER MISMATCH: the first list-item's #label still reads 'MC-821'"* — our build has always rendered `row.mcLabel`, and a test has asserted `items[0].label !== 'MC-821'` since batch 5. Theirs to clear. | n/a |
