...
| `/api/v1/trello/cycle-time/model?rtProjectId=` | v1, key-gated, added 2026-09-10 (Ares #03/#04) | **The cycle-time model Sirius reads** (T180, block 8): one cell per work-type × difficulty `{workType, difficulty, source project\|firm, n, meanWorkingDays, p70, p85, p95, meanCalendarHours}`, plus the envelope `{generatedAt, window, floorMinutes, overrideDays, workingDayHours, workTypeKeys, historyUnverified, dropped{considered, sampled, reasons}}` and — once #15 ships — `laneCells[]` pooled per `laneKey` (optional in the reader). Durations are WORKING DAYS on Ares's calendar; `workingDayHours` is recorded, never assumed. `rtProjectId` is the BARE integer (`rt-837` → 400; unmapped id → 404). Recomputed once per Manila day, byte-stable between. Behind the adapter (`cycleTimeModel`): Zod-parsed, re-asserts `considered === sampled + Σreasons`, null on any failure. One call per project per night |
| `/api/v1/trello/boards/{boardId}/lanes` | v1, key-gated, added 2026-09-10 (Ares #03/#04) | **Apollo's id-keyed lane table** for the lane reconcile check (owl #13): `{boardId, projectName, source, syncedAt, lanes[{listId, name, pos, type work\|process, group, isStart, isDone}]}`. Type and group only — STATE stays product's name table (§7a). `syncedAt` null = table never stamped; **404 = board never synced, not "no lanes"**. Behind the adapter (`boardLanes`): null on any failure incl. 404. Once per board per sync run |
