...
- ⚠️ **`lastPolledAt` IS THE FIELD THAT SOLVES THIS, and BOTH writers stamp it.** `buildCardDoc` sets `lastPolledAt: now`, and the poll and the webhook worker share that one function — so it is a true *fetch* time from Trello, not a poll marker and not a last-modified. Confirmed present in a live response. **Any Sirius logic needing "how old is this data" must read `lastPolledAt`, never the instant the request was issued** (see `worker/syncAres.ts` `staleGuard`). It is right regardless of cadence, which is exactly why it beats assuming a number.
- ⚠️ **Two places `lastPolledAt` deliberately stops advancing** — both fail SAFE for a staleness guard (they look older than they are, so a guard skips rather than over-writes), but know them: **done cards are skipped by the poll** once their history is captured (`isCapturedDoneCard`; live health shows `doneSkipped: 279` and `2674` on two boards), so a done card's stamp only moves if a webhook fires for it; and **`markArchived` writes only `archived`/`archivedAt`**.
