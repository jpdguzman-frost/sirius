...
2. **Read-only everywhere except the write registry.** (Amended 2026-08-04; registry grown 2026-08-12 and 2026-09-10.) No write path to Google Sheets, ever. No write to Trello except the enumerated write registry in `specs/001-sirius-v1/contracts/trello-write.md` — today exactly four entries: the `Urgent` label (`setUrgency()`), the card due date (`setDue()`), the `Difficulty: …` label (`setDifficulty()`, approved by JP 2026-08-12 per BRD-§9-A1) — all via `lib/trello.ts` — and **W4, the business-unit classification label** (approved by JP 2026-09-10 per BRD v2.9 §9, owls #94–#95): assign or remove an EXISTING board label only, matched exactly after case/whitespace normalisation, never fuzzy, never created by Sirius; an unmatched value is refused and surfaced. W4's surface (tab, card kind) and the actor of an ingestion-triggered tag are UNRULED — no code writes W4 until they are. Growing the registry is a constitution amendment, never a code change. If you find yourself writing anything else to a source system, you have misread the task — stop.
